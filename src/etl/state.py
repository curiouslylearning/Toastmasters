...
def get_source_max(url: str) -> date:
    """
    1a: Läs senaste 'Calendar Year' och 'Month' från hemsidan/API.
    """
    end_TM_year = get_years_from_page(url)[0]
    end_month_abbr = get_months_from_page(url)[0]
...
def get_source_min(url: str) -> date:
    """
    Only for use if their is not yet a SQL table, or the table is empty.
    """
    start_TM_year = get_years_from_page(url)[-1]
    end_month_abbr = get_months_from_page(url)[-1]

    start_year = program_year_to_calendar_year(start_TM_year, end_month_abbr)
    end_month = MONTH_MAP[end_month_abbr]

    return start_year, end_month



def get_sql_max(main_table: str) -> date:
    """
    ...
    sql = text(f"""
          ...
          FROM "{main_table}"
          GROUP BY "TM_Year", "CalendarYear", "Month_Num", "Month_Num_Fiscal"
          ORDER BY "TM_Year" ASC, "Month_Num_Fiscal" ASC; \
          """)
...
def build_download_plan(org_level_short: str, url: str, main_table: str, sql_table_exists: str) -> pd.DataFrame:
    """
    ...
    print("sql_table_exists: " + sql_table_exists)

    if sql_table_exists == "Yes":
        start_year, start_month = get_sql_max(main_table)  # outputs start_year, start_month
        end_year, end_month = get_source_max(url) #outputs end_year, end_month

    else:
        start_year, start_month = get_source_min(url)
        end_year, end_month = get_source_max(url)  # outputs end_year, end_month
    ...
    for i in range(len(df_year_month_update_range)):
        ...
        print(
            f"""Looping, instance {i} of {len(df_year_month_update_range)}, month_num: {month_num}, program_year: {program_year}. Storing from pages District, TM_Year, Month_Num, AsOf""")

        if not (month_num == 7 and program_year == "2016-2017"): #exclude as this does not exist in dashboard

            #program_year = str(int(program_year) - 1) + "-" + program_year
            # hämtar distriktlistan
            dfDistricts = get_districts_from_page(org_level_short, program_year, month_num, district_id="01")
